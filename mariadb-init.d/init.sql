...
SET MYSQL_ERRNO = 30001, MESSAGE_TEXT =
...
CREATE OR REPLACE TRIGGER after_insert_usuarios
    ...
    FOR EACH ROW
BEGIN
INSERT INTO palabras_clave (id_usuario)
VALUES (NEW.id);
END;
@@
...
IF LENGTH(v_contrasena) = 0 THEN
        SIGNAL SQLSTATE '45000'
SET MYSQL_ERRNO = 30001, MESSAGE_TEXT = 'La contrasena no puede estar vacia';
END IF;
...
IF id_usuario IS NULL THEN
        SIGNAL SQLSTATE '45000'
SET MYSQL_ERRNO = 30001, MESSAGE_TEXT = 'Usuario no encontrado';
END IF;
RETURN id_usuario;
END;
@@
...
IF id_usuario IS NULL THEN
        SIGNAL SQLSTATE '45000'
SET MYSQL_ERRNO = 30001, MESSAGE_TEXT = 'Usuario o contrasena invalidos';
END IF;
RETURN id_usuario;
END;
@@
...
WHERE usuarios.id = v_id_usuario
  AND usuarios.contrasena = ENCRYPT(v_contrasena, usuarios.contrasena_salt) LIMIT 1;
IF ROW_COUNT() != 1 THEN
        SIGNAL SQLSTATE '45000'
SET MYSQL_ERRNO = 30001, MESSAGE_TEXT = 'Credenciales invalidas';
END IF;
END;
@@
...
SET usuario_activo = (SELECT usuarios.activo
                      FROM usuarios
...
WHERE pc.id_usuario = v_id_usuario LIMIT 1;
END;
@@
